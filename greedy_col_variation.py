import networkx as nx
import graph1
import graph2
import graph3
import graph4
import graph5


def find_next_vertex(G):
    global visitedNodes

    # Find the neigbours of the visited nodes that haven't been coloured yet
    # I use set() as it removes repeated neigbours
    unvisitedNeighbours = sorted(set(
        neighbour
        # For all the visited nodes
        for node in visitedNodes
        # For all the neighbours of that visited node
        for neighbour in G[node]
        if G.nodes[neighbour]["colour"] == "never coloured"
    ))

    # If there are no more neighbours to visit
    if len(unvisitedNeighbours) == 0:
        nextNode = None
    else:
        nextNode = unvisitedNeighbours[0]

    return nextNode


def find_smallest_colour(G, i):
    # Finds the set of colours currently used by the nodes neighbours
    # I use set() as it removes repeated colours
    neighbourColours = sorted(set(
        G.nodes[neighbour]["colour"]
        for neighbour in G[i]
        if G.nodes[neighbour]["colour"] != "never coloured"
    ))

    # Chooses the lowest possible value as the colour to use
    colour = next(
        index
        # The [None] is in the case that the list of neigbour colours is empty.
        for index, value in enumerate(neighbourColours + [None], 1)
        if index != value
    )

    return colour


def greedy(G):
    global visitedNodes
    kmax = 0

    visitedNodes = []
    nextNode = 1
    while nextNode is not None:
        currentNode = nextNode

        colour = find_smallest_colour(G, currentNode)
        # If the maximum colour has to be increased, increase it
        if colour > kmax:
            kmax = colour

        G.nodes[currentNode]["colour"] = colour
        visitedNodes.append(currentNode)
        # Decide which node to visit next
        nextNode = find_next_vertex(G)

# region Lecturer's Code
    print()
    for i in G.nodes():
        print('vertex', i, ': color', G.nodes[i]['colour'])
    print()
    print('The number of colors that Greedy computed is:', kmax)
    print()


print('Graph G1:')
G=graph1.Graph()
G.add_nodes_from(G.nodes(), visited = 'no')
greedy(G)

print('Graph G2:')
G=graph2.Graph()
G.add_nodes_from(G.nodes(), visited = 'no')
greedy(G)

print('Graph G3:')
G=graph3.Graph()
G.add_nodes_from(G.nodes(), visited = 'no')
greedy(G)

print('Graph G4:')
G=graph4.Graph()
G.add_nodes_from(G.nodes(), visited = 'no')
greedy(G)

print('Graph G5:')
G=graph5.Graph()
G.add_nodes_from(G.nodes(), visited = 'no')
greedy(G)

# endregion Lecturer's Code

